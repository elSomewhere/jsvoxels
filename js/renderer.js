...
export class Renderer {
    ...
    renderChunks(meshes, projectionMatrix, viewMatrix) {
        ...
        for (const mesh of meshes) {
            ...
            if (!this.isMeshInFrustum(mesh, projViewMatrix)) {
                skippedChunks++;
                continue;
            }
        ...
        if (DEBUG) {
            debugLog(`Rendered ${drawnChunks} chunks, skipped ${skippedChunks} chunks outside frustum`);
        }
    ...
    isMeshInFrustum(mesh, projViewMatrix) {
        ...
        const minX = mesh.worldOffset[0];
        const minY = mesh.worldOffset[1];
        const minZ = mesh.worldOffset[2];
        const maxX = minX + CHUNK_SIZE;
        const maxY = minY + CHUNK_SIZE;
        const maxZ = minZ + CHUNK_SIZE;

        return mat4.isBoxInFrustum(this.currentFrustum, minX, minY, minZ, maxX, maxY, maxZ);
    }
}