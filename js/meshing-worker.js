...
export class ThreadedChunkManager {
    constructor(renderer) {
        ...
        // Enhanced path resolution - get the complete base URL
        const baseUrl = new URL('./', window.location.href).href;
        console.log('Base URL for worker scripts:', baseUrl);

        const terrainWorkerUrl = baseUrl + 'terrain-worker.js';
        const meshingWorkerUrl = baseUrl + 'meshing-worker.js';

        console.log('Terrain worker URL:', terrainWorkerUrl);
        console.log('Meshing worker URL:', meshingWorkerUrl);

        // Verify worker files exist before creating workers
        this.verifyWorkerFiles(terrainWorkerUrl, meshingWorkerUrl)
            .then(({ terrainExists, meshingExists }) => {
                if (!terrainExists || !meshingExists) {
                    console.error('Worker files not found. Check your file paths.');
                    return;
                }

                // Initialize workers once we confirm files exist
                this.terrainWorkers = new WorkerPool(terrainWorkerUrl, 2);
                this.meshingWorkers = new WorkerPool(meshingWorkerUrl, 2);

                // Initialize workers
                this.initializeWorkers();
            })
            .catch(error => {
                console.error('Error verifying worker files:', error);
            });
    }

    async verifyWorkerFiles(terrainWorkerUrl, meshingWorkerUrl) {
        try {
            const terrainResponse = await fetch(terrainWorkerUrl, { method: 'HEAD' });
            const meshingResponse = await fetch(meshingWorkerUrl, { method: 'HEAD' });

            console.log('Terrain worker exists:', terrainResponse.ok, terrainResponse.status);
            console.log('Meshing worker exists:', meshingResponse.ok, meshingResponse.status);

            return {
                terrainExists: terrainResponse.ok,
                meshingExists: meshingResponse.ok
            };
        } catch (error) {
            console.error('Error fetching worker files:', error);
            return { terrainExists: false, meshingExists: false };
        }
    }
    ...
    updateChunks(playerX, playerY, playerZ) {
        ...
        if (this.terrainWorkers && this.meshingWorkers) {
    ...
    buildChunkMeshes() {
        if (!this.meshingWorkers) return; // Skip if workers aren't initialized yet
    ...
    dispose() {
        if (this.terrainWorkers) this.terrainWorkers.terminate();
        if (this.meshingWorkers) this.meshingWorkers.terminate();
    }
}